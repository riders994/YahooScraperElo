...
class SeasonalFrameCalculator:
    ...
    def _set_k(self, new_k):
        if isinstance(new_k, int) or isinstance(new_k, float):
            if new_k > 0:
                self.k = new_k
...
#                 _logger.info('Calculating for %s vs. %s', player_1, player_2)
                p1_elo = self.team_elo_frame.loc[player_1_id, last_week] if player_1_id in self.team_elo_frame.index else 1500.0
                p2_elo = self.team_elo_frame.loc[player_2_id, last_week] if player_2_id in self.team_elo_frame.index else 1500.0
                player_1_data = [p1_elo * 1.0, true_scores[player_1_id]]
                player_2_data = [p2_elo * 1.0, true_scores[player_2_id]]
...
#         _logger.info('Writing to frame')
    ...
    def run(self, week=0, scoreboard=None, team_elo=None, stat_elo=None, roto=None, year=None, k=None):
        if k:
            self._set_k(k)
        if week:
