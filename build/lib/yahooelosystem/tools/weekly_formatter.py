...
# Maps (league_id, raw_guid) -> replacement guid for deleted accounts specific to one league
LEAGUE_GUID_OVERRIDES = {
    ('364.l.32235', '--'): 'JEREMYPLACEHOLDER2016XXXXX',
}
    ...
    def _get_guid(self, team):
        for d in team[0]:
            if isinstance(d, dict):
                ...
                res = d.get('managers')
                if res:
                    guid = res[0]['manager']['guid']
                    override = LEAGUE_GUID_OVERRIDES.get((self.league_id, guid))
                    if override:
                        return override
                    return guid
